...
gmd(
  ...
  async (from, Gifted, conText) => {
    ...
    try {
      if (!isSuperUser) return reply(stylishReply("*Owner Only Command*"));
      if (!q)
        return reply(
      ...
      if (reportedMessages[messageId]) {
        return reply(stylishReply("This report has already been forwarded to the owner. Please wait for a response.",
        ));
      }
...
┗▣`;
      ...
      const giftedMess = {
        image: { url: botPic },
        caption: stylishReply(menus.trim()),
        contextInfo: {
          mentionedJid: [sender],
          forwardingScore: 5,
          isForwarded: true,
...
┗▣${readmore}\n`;
      ...
      const giftedMess = {
        image: { url: botPic },
        caption: stylishReply(list.trim()),
        contextInfo: {
          mentionedJid: [sender],
          forwardingScore: 5,
          isForwarded: true,
